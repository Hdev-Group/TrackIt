"use client"
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';
import Link from 'next/link';
import Button from '@/components/button/button';
import SignIn from '@/components/signin/signin';
import { AlertOctagon, Clock, Columns3, Lock } from 'lucide-react';

export default function Page() {
  const router = useRouter();
  const [redirectUrl, setRedirectUrl] = useState<string | null>(null);
  const [currentColorIndex, setCurrentColorIndex] = useState(0);
  const [rotation, setRotation] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [imageState, setImageState] = useState('entering');

  const colourgradientsandimages = [
    { color: '#00ffff', image: <TrackitWhite />}, // black
    { color: '#0088ff', image: <Columns3 className='h-14 w-14' /> }, // Ocean Blue
    { color: '#4b0082', image: <AlertOctagon className='h-14 w-14' /> }, // Indigo
    { color: '#6a5acd', image: <Clock className='h-14 w-14' /> }, // Slate Blue
  ];

  function TrackitWhite() {
    return(
      <img src="/trackitlogo/light/logo.png" className='w-auto h-10'></img>
    )
  }

  useEffect(() => {
    const searchParams = new URLSearchParams(window.location.search);
    const redirectUrl = searchParams.get('redirect');
    if (redirectUrl) {
      setRedirectUrl(redirectUrl);
    }
  }, []);

  useEffect(() => {
    const interval = setInterval(() => {
      setImageState('leaving'); 

      setTimeout(() => {
        setRotation((prev) => prev + 90);
        setCurrentColorIndex((prevIndex) => (prevIndex + 1) % colourgradientsandimages.length);
        setCurrentImageIndex((prevIndex) => (prevIndex + 1) % colourgradientsandimages.length);

        setImageState('entering');
      }, 500);
    }, 2500);

    return () => clearInterval(interval);
  }, [colourgradientsandimages.length]);

  const currentColor = colourgradientsandimages[currentColorIndex].color;
  const currentImage = currentImageIndex === 4 ? '/IMG_6128.png' : colourgradientsandimages[currentImageIndex].image;

  return (
    <body>
      <title>Sign In | TrackIt</title>
      <main className="flex flex-col md:flex-row justify-between items-start overflow-hidden bg-background">
      <div className='mx-auto md:px-10 flex flex-col w-full'>
        <div className='w-full h-20 md:px-0 px-3 py-3 flex items-center justify-between'>
            <a href='/'>
              <img src="/trackitlogo/light/wordmark.png" className='w-auto h-10'></img>
            </a>            
            <div className='flex flex-row gap-4 items-center'>
              <p className='text-muted-foreground text-sm'>New to TrackIt?</p>
              <Button
                variant='ghost-side-fill'
                className='!border-muted'
                onClick={() => router.push('/auth/signup')}
                >
                Sign Up
                </Button>
            </div>
          </div>
          <div className='w-full rounded-t-2xl bg-gradient-to-t flex items-center flex-col h-full from-background to-[#1f1f23] p-8'>
            <div className='flex flex-col items-center md:mt-[3%]'>
              <div className='flex items-center justify-center px-8 mb-4 py-1 rounded-lg text-[#6fcf97] text-md border border-[#6fcf97]/40 border-t-[#6fcf97] bg-[#304a36] font-normal '>
                <Lock className='mr-2 h-5 w-5' />
                Sign In
              </div>
              <div className='relative w-20 h-20 overflow-hidden bg-transparent flex items-center justify-center'>
                <div
                  className='w-16 h-16 rounded-xl overflow-hidden z-20 m-4 absolute flex items-center justify-center'
                  id='spinzoner'
                  style={{
                  transform: `rotate(${rotation}deg)`,
                  backgroundColor: rotation % 360 === 0 ? '#111114' : currentColor,
                  transition: 'transform 0.5s, background-color 0.5s',
                  }}
                />
                <div className='w-14 h-14 overflow-hidden flex items-center justify-center'>
                  <div
                    className={`w-14 h-14 flex items-center justify-center z-50 p-1.5 transition-transform duration-500 ease-in-out ${
                    imageState === 'entering'
                    ? 'translate-x-[0%]'
                    : 'translate-x-[100%]'
                    }`}
                  >
                    {currentImage}
                  </div>
                </div>
              </div>
              <h1 className='text-[40px] md:text-[63px] font-semibold text-white'>Welcome back to TrackIt</h1>
              <SignIn />
            </div>
          </div>
        </div>
      </main>
    </body>
  );
}
